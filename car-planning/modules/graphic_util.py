...
def reconstruct_path(came_from, current, draw, start_node):
    while current in came_from:
        ...
        if (
                current.get_pos() == start_node.get_pos()):  # if we reached the start node we don't want to color it purple, so we ignore it
            continue
        current.make_path()
        draw()
...
def save_grid_to_file(grid):
    ...
    with open(os.path.join(script_dir, '..', 'grid_file'), 'wb') as f:
        pickle.dump(grid, f)


def load_grid_from_file(map_name):
    script_dir = os.path.dirname(os.path.realpath(__file__))
    with open(os.path.join(script_dir, '..', map_name), 'rb') as f:
        grid = pickle.load(f)
        return grid
